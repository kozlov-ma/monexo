...
class InMemoryUserRepository(UserRepositoryBase):
    ...
    async def update_user_partially(self, id: int, days_left: int = None, whole_budget: float = None,
                                    expense_today: float = None, income_today: float = None) -> Result[None, Exception]:
        if id not in self.dict:
            return Err(KeyError(f"User with id {id} does not exist"))

        kwargs = {k: v for k, v in
                  {'days_left': days_left, 'whole_budget': whole_budget, 'expense_today': expense_today,
                      'income_today': income_today}.items() if v is not None}
...
class PostgresUserRepository(UserRepositoryBase):
    ...
    async def get_by_id(self, id: int) -> Option[User]:
        statement = (select(DbUser).where(DbUser.telegram_id == id))
    ...
    async def add_user(self, user: User) -> Result[None, Exception]:
        statement = (select(DbUser).where(DbUser.telegram_id == user.id))
    ...
    async def update_user_partially(self, id: int, days_left: int = None, remaining_budget: float = None,
                                    budget_today: float = None, expense_today: float = None,
                                    income_today: float = None) -> Result[None, Exception]:
        statement = (select(DbUser).where(DbUser.telegram_id == id))
